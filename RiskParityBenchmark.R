...
GetBiasInfoRpt <- function(complete.ts, end.date, is.abs.change=FALSE, period='year', is.abs.change=FALSE,
                            n=1, median.as.mean=TRUE, log.rt=TRUE, sample.history.years=-1) {
  bias <- NA
  sample <- NULL
  if(is.abs.change) {
    bias <- complete.ts[end.date]
    sample <- coredate(complete.ts[paste('/', end.date, sep = '')])
  } else {
